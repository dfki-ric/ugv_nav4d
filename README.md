...
TODO picuture of primitives here
...
The motion primitives used by the planner are generated by the `SbplSplineMotionPrimitives` package. They are filtered based on `config.minTurningRadius` (i.e. all primitives that have a curvature that is larger than allowed by the minimum turning radius are ignored) and converted into `Motion` objects.
