...
void TraversabilityGenerator3d::dijkstraComputeCostNoDoubleFrontierts(const TravGenNode* source, std::vector< double >& outDistances, const double maxDist) const
{
    ...
    struct CostWithHit
    {
        CostWithHit(double dist) : distance(dist), frontiersHit(0)
        {
        }
        CostWithHit() : distance(0.0), frontiersHit(0)
        {
        }
        bool operator <(const CostWithHit &o) const
        {
    ...
    std::set<std::pair<CostWithHit, const TravGenNode*>> vertexQ;
    vertexQ.insert(std::make_pair( outDistances[sourceId], source));
    
   
    while (!vertexQ.empty()) 
    {
        CostWithHit nodeInfo = vertexQ.begin()->first;
        ...
        {   
            ...
            double distance_through_u = nodeInfo.distance + distance;
